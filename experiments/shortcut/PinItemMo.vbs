...
Function PinItem(strlPath, blnTaskbar)
    ...
    strFolder = objFSO.GetParentFolderName(objFSO.GetFolder(strlPath))
    ...
    Else
        For each itemverb in objFolderItem.verbs
            If Replace(itemverb.name, "&", "") = "Pin to Start Menu" or Replace(itemverb.name, "&", "") = "Pin to Start" Then itemverb.DoIt
        Next 
    End If
